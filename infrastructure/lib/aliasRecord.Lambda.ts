...
export async function handler(event: any): Promise<any> {
  ...
  switch (event.RequestType) {
    ...
    case "Update":
      await route53
        .changeResourceRecordSets(
          generateRecordChangeParams("UPSERT", hostedZoneId, domain, targetAlias, targetHostedZoneId)
        )
        .promise();
    ...
    default:
      await route53
        .changeResourceRecordSets(
          generateRecordChangeParams("DELETE", hostedZoneId, domain, targetAlias, targetHostedZoneId)
        )
        .promise();
  }
}
